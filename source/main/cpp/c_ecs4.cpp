...
namespace ncore
{
    namespace necs4
    {
        ...
        static inline entity_t s_entity_make(entity_generation_t genid, u8 shard_index, entity_index_t entity_index)
        {
            ...
            return ((u32)genid << ECS_ENTITY_GEN_ID_SHIFT) | ((u32)shard_index << ECS_ENTITY_SHARD_SHIFT) | ((u32)entity_index & ECS_ENTITY_INDEX_MASK);
        }
...
#define ECS_SHARD_MAX_ENTITIES 65536
        ...
        struct shard_t
        {
            ...
            u32             m_reserved_pages;                    // total number of reserved pages for this shard
        ...
        static shard_t* s_create(u32 max_component_types, u32 max_components_per_entity, u32 max_tags_per_entity)
        {
            ...
            shard->m_cp_global_occupancy               = (arena_t*)(shard + 1);
            shard->m_cp_local_occupancy                = shard->m_cp_global_occupancy + 1;
            shard->m_cp_indexed                        = shard->m_cp_local_occupancy + 1;
            shard->m_cp_reference                      = shard->m_cp_indexed + 1;
            shard->m_tags                              = shard->m_cp_reference + 1;
            shard->m_reserved_pages                    = (u32)(total_size >> page_size_shift);
            shard->m_component_bins                    = (nbin16::bin_t**)(shard->m_tags + 1);
            ...
            byte* arena_memory_base = base_address + shard_size;
            narena::init_arena(shard->m_cp_global_occupancy, arena_memory_base, global_occupancy, 0);
            arena_memory_base += global_occupancy;
            narena::init_arena(shard->m_cp_local_occupancy, arena_memory_base, local_occupancy, 0);
            arena_memory_base += local_occupancy;
            narena::init_arena(shard->m_cp_indexed, arena_memory_base, indexed_array, 0);
            arena_memory_base += indexed_array;
            narena::init_arena(shard->m_cp_reference, arena_memory_base, reference_array, 0);
            arena_memory_base += reference_array;
            narena::init_arena(shard->m_tags, arena_memory_base, tags_array, 0);

            g_memclr(shard->m_component_bins, component_bins_array_size);

            return shard;
        }

        static void s_destroy(shard_t* shard)
        {
            if (shard != nullptr)
            {
                // free all component bins
                for (u16 i = 0; i < shard->m_max_component_types; i++)
                {
                ...
                // free all arenas
                narena::destroy(shard->m_cp_global_occupancy);
                narena::destroy(shard->m_cp_local_occupancy);
                narena::destroy(shard->m_cp_indexed);
                narena::destroy(shard->m_cp_reference);
                narena::destroy(shard->m_tags);

                v_alloc_release((byte*)shard, shard->m_reserved_pages << shard->m_cp_global_occupancy->m_page_size_shift);
            }
        }
        ...
        static byte* s_alloc_component(shard_t* shard, u32 entity_index, u16 component_type_index)
        {
            ...
            else
            {
                ...
                if (cp_ptr != nullptr)
                {
                    ...
                    if (cp_local_index < 0)
                        return nullptr;
                    const u16 cp_reference                 = nbin16::ptr2idx(cp_bin, cp_ptr);
                    u16*      cp_reference_array           = (u16*)(shard->m_cp_reference->m_base + (entity_index * shard->m_per_entity_reference_bytes));
                    ...
                    cp_local_occupancy[cp_local_index >> 3] |= (1u << (cp_local_index & 7));
                    return (byte*)cp_ptr;
                }
            }
            return nullptr;
        ...
        static void s_free_component(shard_t* shard, u32 entity_index, u16 component_type_index)
        {
            ...
            if (cp_global_occupancy[byte_index] & bit_mask)
            {
                ...
                u8*      cp_indices              = shard->m_cp_indexed->m_base + (entity_index * shard->m_per_entity_indexed_bytes);
                const u8 cp_index                = cp_indices[component_type_index];
                cp_indices[component_type_index] = 0xFF; // invalidate index
                u16*      cp_references          = (u16*)(shard->m_cp_reference->m_base + (entity_index * shard->m_per_entity_reference_bytes));
                const u16 cp_reference           = cp_references[cp_index];
                cp_references[cp_index]          = 0xFFFF; // invalidate reference
                ...
                u8* cp_local_occupancy = shard->m_cp_local_occupancy->m_base + (entity_index * shard->m_per_entity_local_occupancy_bytes);
                cp_local_occupancy[cp_index >> 3] &= ~(1u << (cp_index & 7));
            }
        }
        ...
        static byte* s_get_component(shard_t* shard, u32 entity_index, u16 component_type_index)
        {
            ...
            if (global_occupancy[byte_index] & bit_mask)
            {
                ...
                nbin16::bin_t* cp_bin = shard->m_component_bins[component_type_index];
                ASSERT(cp_bin != nullptr);
                const u16* cp_references = (u16*)(shard->m_cp_reference->m_base + (entity_index * shard->m_per_entity_reference_bytes));
                const u16  cp_reference  = cp_references[cp_index];
                return (byte*)nbin16::idx2ptr(cp_bin, cp_reference);
            }
            return nullptr;
        ...
        struct ecs_t
        {
            ...
            u8*       m_shards_sorted;
            shard_t** m_shards; // array of shards
        };
        // ecs_t is followed in memory by:
        // u8                 m_shards_sorted[]; // sorted array of shard indices, to always have an active shard at index 0
        // shard_t*   m_shards[];       // array of shard pointers

        ecs_t* g_create_ecs(u16 components_per_entity, u16 max_component_types, u16 max_tag_types)
        {
            const u8  page_size_shift = v_alloc_get_page_size_shift();
            const u32 page_size       = (u32)1 << page_size_shift;

            const int_t ecs_size = math::alignUp(sizeof(ecs_t) + (sizeof(u8) * 256) + (sizeof(shard_t*) * 256), page_size);
            ...
            ecs->m_shards          = (shard_t**)(ecs->m_shards_sorted + ecs->m_shards_capacity);

            // initialize shards array
            g_memclr(ecs->m_shards_sorted, sizeof(u8) * ecs->m_shards_capacity);
            g_memclr(ecs->m_shards, sizeof(shard_t*) * ecs->m_shards_capacity);

            // create first shard
            ecs->m_shards_sorted[0] = 0;
            ecs->m_shards[0]        = s_create(max_component_types, components_per_entity, max_tag_types);
        ...
        entity_t g_create_entity(ecs_t* ecs)
        {
            ...
            shard_t*  shard        = ecs->m_shards[shard_index];
            const s32 entity_index = s_create_entity(shard);
        ...
        void g_destroy_entity(ecs_t* ecs, entity_t e)
        {
            ...
            shard_t* shard       = ecs->m_shards[shard_index];
            s_destroy_entity(shard, g_entity_index(e));
        }
        ...
        void g_register_component_type(ecs_t* ecs, u16 cp_index, u32 cp_sizeof)
        {
            ...
            {
                shard_t* shard = ecs->m_shards[i];
                s_register_component_type(shard, cp_index, cp_sizeof);
            }
        }
        ...
        bool g_has_cp(ecs_t* ecs, entity_t entity, u32 cp_index)
        {
            ...
            shard_t* shard       = ecs->m_shards[shard_index];
            return s_has_component(shard, g_entity_index(entity), (u16)cp_index);
        }
        void* g_add_cp(ecs_t* ecs, entity_t entity, u32 cp_index)
        {
            const u8 shard_index = g_entity_shard_index(entity);
            shard_t* shard       = ecs->m_shards[shard_index];
            return s_alloc_component(shard, g_entity_index(entity), (u16)cp_index);
        }
        void g_rem_cp(ecs_t* ecs, entity_t entity, u32 cp_index)
        {
            const u8 shard_index = g_entity_shard_index(entity);
            shard_t* shard       = ecs->m_shards[shard_index];
            s_free_component(shard, g_entity_index(entity), (u16)cp_index);
        }
        void* g_get_cp(ecs_t* ecs, entity_t entity, u32 cp_index)
        {
            const u8 shard_index = g_entity_shard_index(entity);
            shard_t* shard       = ecs->m_shards[shard_index];
            return s_get_component(shard, g_entity_index(entity), (u16)cp_index);
        }
        ...
        bool g_has_tag(ecs_t* ecs, entity_t entity, u16 tg_index)
        {
            const u8 shard_index = g_entity_shard_index(entity);
            shard_t* shard       = ecs->m_shards[shard_index];
            return s_has_tag(shard, entity, tg_index);
        }
        ...
        void g_add_tag(ecs_t* ecs, entity_t entity, u16 tg_index)
        {
            const u8 shard_index = g_entity_shard_index(entity);
            shard_t* shard       = ecs->m_shards[shard_index];
            s_add_tag(shard, entity, tg_index);
        }
        ...
        void g_rem_tag(ecs_t* ecs, entity_t entity, u16 tg_index)
        {
            const u8 shard_index = g_entity_shard_index(entity);
            shard_t* shard       = ecs->m_shards[shard_index];
            s_rem_tag(shard, entity, tg_index);
        }
        ...
        {
            //
            return m_entity_index >= 0 ? s_entity_make(0, m_shard_index, m_entity_index) : ECS_ENTITY_NULL;
        }
        ...
        s32 en_iterator_t::find(s32 entity_index) const
        {
            ...
            {
                return entity_index >= 0 ? nduomap18::find1_after(&m_shard->m_free_bin0, m_shard->m_free_bin1, &m_shard->m_alive_bin0, m_shard->m_alive_bin1, m_shard->m_bin2, m_shard->m_free_index, entity_index) : -1;
            }
            ...
            u8 const  ref_shard_index         = g_entity_shard_index(m_entity_reference);
            shard_t*  ref_shard               = m_ecs->m_shards[ref_shard_index];
            u8 const* ref_component_occupancy = &ref_shard->m_cp_global_occupancy->m_base[m_entity_reference * m_shard->m_per_entity_global_occupancy_bytes];
            u8 const* ref_tag_occupancy       = &ref_shard->m_tags->m_base[m_entity_reference * m_shard->m_per_entity_tags_bytes];
            s32 const ref_entity_index        = g_entity_index(m_entity_reference);

            entity_index = nduomap18::find1_after(&m_shard->m_free_bin0, m_shard->m_free_bin1, &m_shard->m_alive_bin0, m_shard->m_alive_bin1, m_shard->m_bin2, m_shard->m_free_index, entity_index);
            while (entity_index >= 0)
            {
                if (entity_index != ref_entity_index)
                {
                    u8 const* cur_component_occupancy = &m_shard->m_cp_global_occupancy->m_base[entity_index * m_shard->m_per_entity_global_occupancy_bytes];
                    bool      has_all_components      = true;
                    for (u32 i = 0; i < m_shard->m_per_entity_global_occupancy_bytes; ++i)
                    {
                    ...
                    {
                        u8 const* cur_tag_occupancy = &m_shard->m_tags->m_base[entity_index * m_shard->m_per_entity_tags_bytes];
                        bool      has_all_tags      = true;
                        for (u32 i = 0; i < m_shard->m_per_entity_tags_bytes; ++i)
                        {
