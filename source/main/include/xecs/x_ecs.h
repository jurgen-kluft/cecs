...
namespace xcore
{
    ...
    struct cp_type_t
    {
        inline cp_type_t(u32 id, u32 sizeofcp, const char *name)
            : cp_id(id), cp_sizeof(sizeofcp), cp_name(name)
        {
        }
        u16 const cp_id;
        u16 const cp_sizeof;
    ...
    template <typename T>
    inline const char *nameof() { return "?"; }

    template <typename T>
    cp_type_t g_register_component_type(ecs_t *r) { return g_register_component_type(r, sizeof(T), nameof<T>()); }
